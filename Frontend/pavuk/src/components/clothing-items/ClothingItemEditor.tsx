...
import ClothingItemType, {
  ClothingItemTypeEnum,
} from "../../types/clothing-item-type";
import FabricType from "../../types/fabric-type";
import ColorType from "../../types/color-type";
import axios from "axios";
import "./ClothingItemEditor.css";
import ShippingInfoForm from "../orders/ShippingInfoForm";
import {
  showShippingInfoForm,
  hideShippingInfoForm,
} from "../../state/ui/ui-slice";
import { shippingInfoNotValid, updateShippingInfo } from "../../state/shipping-info/shipping-info-slice";
...
function ClothingItemEditor() {
  ...
  const shippingInfo = useSelector((state: RootState) => state.shippingInfo.shippingInfo);
  const shippingInfoValid = useSelector((state: RootState) => state.shippingInfo.shippingInfoValid);
  const isShippingInfoFormVisible = useSelector(
    (state: RootState) => state.ui.isShippingFormVisible
  );
  ...
  const primaryImage = Array.from(clothingItem.images).find(
    (image) => image.isPrimary
  );
  const primaryImageUrl = primaryImage
    ? primaryImage.url
    : Array.from(clothingItem.images)[0]!.url;
  const fabrics: Array<FabricType> = useSelector(
    (state: RootState) => state.fabrics.fabrics
  );

  const [primaryImageSvg, setPrimaryImageSvg] = useState<string>("");
  const [primaryImageSvgLayers, setPrimaryImageSvgLayers] = useState<SvgLayers>(
    { svgLayers: {}, svgLayerGroups: {}, svgStyles: "" }
  );
  ...
  const [selectedFabricColor, setSelectedFabricColor] = useState<ColorType>(
    fabricColors[0]
  );
  ...
  const [clothingParts, setClothingParts] = useState<{
    [key: string]: {
      ...
      selectedFabricColor: ColorType | null;
    };
  }>({});
  ...
  useEffect(() => {
    const fetchSvg = async () => {
      ...
      if (svgDoc && svgDocStyle) {
        const svgStyles = svgDoc.querySelector("style")
          ? svgDocStyle.innerHTML
          : "";
  ...
  const changeStrokeColor = (hex: string, amount = 0.2) => {
    ...
    const r = Math.max(
      0,
      Math.min(255, (value >> 16) - Math.round(255 * amount))
    );
    const g = Math.max(
      0,
      Math.min(255, ((value >> 8) & 0x00ff) - Math.round(255 * amount))
    );
    const b = Math.max(
      0,
      Math.min(255, (value & 0x0000ff) - Math.round(255 * amount))
    );
  ...
  const clothingHandler = (clothingPartName: string) => {
    ...
    if (selectedFabric && selectedFabricColor) {
      const clothingEditorImage = document.getElementById(
        "clothing-editor-image"
      );
      if (clothingEditorImage) {
        const clothingEditorImageSvg = clothingEditorImage.querySelector("svg");
        const clothingEditorImageOverlay = document.getElementById(
          "clothing-editor-image-overlay"
        );
        if (clothingEditorImageSvg && clothingEditorImageOverlay) {
          clothingEditorImageOverlay.setAttribute(
            "viewBox",
            clothingEditorImageSvg.getAttribute("viewBox")!
          );

          const patternId = `pattern-${clothingPartName}`;
          const existingPattern = clothingEditorImageOverlay.querySelector(
            `#${patternId}`
          );
          if (existingPattern) {
          ...
          const patternImage = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "image"
          );
          patternImage.setAttribute(
            "href",
            `${import.meta.env.VITE_SERVER_PATH}${selectedFabric.image.url}`
          );
          ...
          if (clothingPartName === clothingEditorImageSvg.id) {
            clothingEditorImage
              .querySelectorAll("path[id]")
              .forEach((clotingItemPart) => {
                ...
                (clotingItemPart as HTMLElement).setAttribute(
                  "style",
                  `fill: ${selectedFabricColor?.hex}`
                );
              });

            clothingEditorImage
              .querySelectorAll(`path[id*="-line"]`)
              .forEach((clotingItemPartLine) => {
                (clotingItemPartLine as HTMLElement).setAttribute(
                  "style",
                  `stroke: ${changeStrokeColor(selectedFabricColor.hex)}`
                );
              });
          } else {
            const path = document.createElementNS(
            ...
            const pathData = clothingEditorImage
              .querySelector(`#${clothingPartName}`)!
              .getAttribute("d")!;
            path.setAttribute("d", pathData);
            path.setAttribute("style", `fill: url(#${patternId})`);
            clothingEditorImageOverlay!.appendChild(path);
            ...
            clothingEditorImage
              .querySelectorAll(`path[id^="${clothingPartName}"]`)
              .forEach((clotingItemPart) => {
                (clotingItemPart as HTMLElement).setAttribute(
                  "style",
                  `fill: ${selectedFabricColor?.hex}`
            ...
            clothingEditorImage
              .querySelectorAll(`path[id^="${clothingPartName}-line"]`)
              .forEach((clotingItemPartLine) => {
                (clotingItemPartLine as HTMLElement).setAttribute(
                  "style",
                  `stroke: ${changeStrokeColor(selectedFabricColor.hex)}`
  ...
  const createOrderHandler = async () => {
    //const shippingInfo = useSelector((state: RootState) => state.ui.shippingInfo); 
...
console.log(shippingInfo);
    const savedSvg = await saveSvgImageById(
      "clothing-editor-image",
      `${currentUser.email}`
    );
    if (savedSvg) {
        ...
        <div className="clothing-editor w-full h-full px-40">
          <div className="h-full aspect-square px-24 py-12">
            <div className="relative h-full w-full">
              <div className="absolute flex items-center aspect-square h-full">
                <svg
                  id="clothing-editor-image-overlay"
                  xmlns="http://www.w3.org/2000/svg"
                ></svg>
              </div>
              <div
                id="clothing-editor-image"
                dangerouslySetInnerHTML={{ __html: primaryImageSvg }}
              ></div>
            </div>
          </div>
          ...
          <div className="w-full py-9">
            <div className="clothing-editor-order-creation h-full w-full">
                  ...
                  <div className="h-0">
                    <div className="flex flex-col px-8 pt-7 pb-4 gap-3">
                      <div className="flex flex-col">
                        <p className="clothing-editor-tools-heading text-base mb-1">
                          Деталі одягу
                        </p>
                        <div className="clothing-editor-clothing-part-container">
                          {Object.entries(
                            primaryImageSvgLayers.svgLayerGroups
                          ).map(([commonName, layerNames]) => (
                            <div
                              key={commonName}
                              className={`clothing-editor-clothing-part box-border w-9 h-9 p-[4px] pt-[5px] ${
                                selectedClothingPart === commonName
                                  ? "clothing-editor-clothing-part-selected"
                                  : ""
                              }`}
                                ...
                                {layerNames.map((layerName) => (
                                  <g
                                    ...
                                    dangerouslySetInnerHTML={{
                                      __html:
                                        primaryImageSvgLayers.svgLayers[
                                          layerName
                                        ].layerData,
                                    }}
                                  />
                            ...
                            <img
                              ...
                              className={`clothing-editor-fabric w-9 h-9 ${
                                selectedFabric?.id === fabric.id
                                  ? "clothing-editor-fabric-selected"
                                  : ""
                              }`}
                              src={`${import.meta.env.VITE_SERVER_PATH}${
                                fabric.image.url
                              }`}
                              alt={fabric.name}
                          ...
                          {fabricColors.map((fabricColor) => (
                            <div
                              ...
                              className={`clothing-editor-color box-border w-9 h-9 ${
                                selectedFabricColor?.id === fabricColor.id
                                  ? "clothing-editor-color-selected"
                                  : ""
                              }`}
              ...
              <div className="flex mt-7 px-8 pt-4 pb-7">
                {isShippingInfoFormVisible && <ShippingInfoForm/>}

                <button
                  className="clothing-editor-button text-lg px-8 py-3"
                  onClick={() => dispatch(showShippingInfoForm())}
                >
                  Заповнити інформацію для доставки
                </button>
              </div>

              <div className="clothing-editor-info">
                <div className="px-8 pt-4 pb-7">
                  <h4 className="clothing-editor-heading text-2xl">
                    {clothingItem.name}
                  </h4>
                  <p className="clothing-editor-subtitle text-lg">
                    {clothingItem.description}
                  </p>
                  <p className="clothing-editor-price mt-2">
                    <span className="text-xl">{clothingItem.price}</span>
                    <span className="clothing-editor-price-currency text-lg">
                      ₴
                    </span>
                  </p>
